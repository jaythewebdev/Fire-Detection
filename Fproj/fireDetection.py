import cv2         # Library for openCVpip
...
def send_mail_function(): # defined function to send mail post fire detection using threading
    ...
    recipientmail = "suryashanmugam637@gmail.com" # recipients mail
    ...
    try:
        ...
        server.ehlo()
        server.starttls()
        server.login("jayasuryam73@gmail.com", 'lbjnsswsejpgulxo') # Senders mail ID and password
        server.sendmail('jayasuryam73@gmail.com', recipientmail, "Warning fire accident has been reported") # recipients mail with mail message
        print("Alert mail sent sucesclfully to {}".format(recipientmail)) # to print in consol to whome mail is sent
